// src/hooks/useSekretariatUnit.js
import { useState, useEffect } from 'react';
import { Platform, Alert } from 'react-native';
import { supabaseSandbox } from '../supabaseSandboxClient';

export function useSekretariatUnit() {
  ...
  const saveUnitItem = async (draft, editItem) => {
    ...
    const { error } = editItem
      ? await supabaseSandbox.from('sekretariat_unit').update(payload).eq('id', editItem.id)
      : await supabaseSandbox.from('sekretariat_unit').insert([{ ...payload, display_order: unitList.length }]);
    if (error) {
  ...
  const deleteUnitItem = async (item) => {
    const doDelete = async () => {
      await supabaseSandbox.from('sekretariat_unit').delete().eq('id', item.id);
      fetchUnit();
    };
    if (Platform.OS === 'web') {
  ...
  const reorderUnit = async (reordered) => {
    ...
    await Promise.all(
      reordered.map((item, index) =>
        supabaseSandbox.from('sekretariat_unit').update({ display_order: index }).eq('id', item.id)
      )
    );
  };
